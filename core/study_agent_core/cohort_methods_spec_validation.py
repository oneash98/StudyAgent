...
def _check_get_db_args(value: Any, violations: List[str]) -> None:
    ...
    for idx, period in enumerate(periods):
        if not isinstance(period, dict):
            violations.append(f"studyPeriods[{idx}] must be an object")
...
def _check_trim_by_ps(value: Any, violations: List[str], prefix: str = "trimByPsArgs") -> None:
    if value is None:
        return
    if not _require_object(prefix, value, violations):
        return
    trim = value.get("trimFraction")
    if isinstance(trim, (int, float)) and (trim < 0 or trim > 1):
        violations.append(f"{prefix}.trimFraction must be between 0 and 1")
    bounds = value.get("equipoiseBounds")
    if bounds is not None:
        if not isinstance(bounds, list) or len(bounds) != 2:
            violations.append(f"{prefix}.equipoiseBounds must be a two-item list or null")
        elif all(isinstance(x, (int, float)) for x in bounds):
            if bounds[0] < 0 or bounds[1] > 1 or bounds[0] >= bounds[1]:
                violations.append(f"{prefix}.equipoiseBounds must be ordered values between 0 and 1")


def _check_match_on_ps(value: Any, violations: List[str], prefix: str = "matchOnPsArgs") -> None:
    if value is None:
        return
    if not _require_object(prefix, value, violations):
        return
    ...
    if isinstance(ratio, (int, float)) and ratio < 0:
        violations.append(f"{prefix}.maxRatio must be >= 0")
    cal = value.get("caliper")
    if isinstance(cal, (int, float)) and cal < 0:
        violations.append(f"{prefix}.caliper must be >= 0")
    scale = value.get("caliperScale")
    if scale is not None and scale not in _CALIPER_SCALE:
        violations.append(f"{prefix}.caliperScale must be one of {sorted(_CALIPER_SCALE)}")


def _check_stratify_by_ps(value: Any, violations: List[str], prefix: str = "stratifyByPsArgs") -> None:
    if value is None:
        return
    if not _require_object(prefix, value, violations):
        return
    ...
    if isinstance(strata, (int, float)) and strata < 1:
        violations.append(f"{prefix}.numberOfStrata must be >= 1")
    base = value.get("baseSelection")
    if base is not None and base not in _BASE_SELECTION:
        violations.append(f"{prefix}.baseSelection must be one of {sorted(_BASE_SELECTION)}")


def _check_ps_settings(value: Any, violations: List[str]) -> None:
    if not isinstance(value, list):
        violations.append("psSettings must be a list")
        return
    for idx, ps in enumerate(value):
        prefix = f"psSettings[{idx}]"
        if not isinstance(ps, dict):
            violations.append(f"{prefix} must be an object")
            continue
        match = ps.get("matchOnPsArgs")
        strat = ps.get("stratifyByPsArgs")
        inverse = bool(ps.get("inversePtWeighting"))
        if match is not None and strat is not None:
            violations.append(f"{prefix} cannot include both matchOnPsArgs and stratifyByPsArgs")
        if inverse and (match is not None or strat is not None):
            violations.append(f"{prefix}.inversePtWeighting cannot be combined with matching or stratification")
        _check_trim_by_ps(ps.get("trimByPsArgs"), violations, f"{prefix}.trimByPsArgs")
        _check_match_on_ps(match, violations, f"{prefix}.matchOnPsArgs")
        _check_stratify_by_ps(strat, violations, f"{prefix}.stratifyByPsArgs")
...
def _check_create_ps(value: Any, violations: List[str]) -> None:
    ...
    if isinstance(max_fit, (int, float)) and max_fit < 0:
        violations.append("createPsArgs.maxCohortSizeForFitting must be >= 0")
...
def _check_outcome_model(value: Any, violations: List[str]) -> None:
    if not _require_object("fitOutcomeModelArgs", value, violations):
        return
    settings = value.get("outcomeModels")
    if not isinstance(settings, list):
        violations.append("outcomeModels must be a list")
        return
    for idx, outcome_model in enumerate(settings):
        if not isinstance(outcome_model, dict):
            violations.append(f"outcomeModels[{idx}] must be an object")
            continue
        model_type = outcome_model.get("modelType")
        if model_type is not None and model_type not in _MODEL_TYPE:
            violations.append(f"outcomeModels[{idx}].modelType must be one of {sorted(_MODEL_TYPE)}")
    control = value.get("control")
    if isinstance(control, dict):
        cv = control.get("cvType")
        if cv is not None and cv not in _CV_TYPE:
            violations.append(f"fitOutcomeModelArgs.control.cvType must be one of {sorted(_CV_TYPE)}")
        noise = control.get("noiseLevel")
        if noise is not None and noise not in _NOISE_LEVEL:
            violations.append(f"fitOutcomeModelArgs.control.noiseLevel must be one of {sorted(_NOISE_LEVEL)}")


_SECTION_CHECKERS = {
    "getDbCohortMethodDataArgs": _check_get_db_args,
    "createStudyPopArgs": _check_study_pop,
    "psSettings": _check_ps_settings,
    "createPsArgs": _check_create_ps,
...
def _derive_outcome_flags(ps_setting: Dict[str, Any]) -> Dict[str, bool]:
    ps_setting = ps_setting or {}
    match_args = ps_setting.get("matchOnPsArgs") or {}
    try:
        max_ratio = int(match_args.get("maxRatio", 1))
    except Exception:
        max_ratio = 1
    has_stratification = ps_setting.get("stratifyByPsArgs") is not None
    has_variable_ratio_match = ps_setting.get("matchOnPsArgs") is not None and max_ratio != 1
    return {
        "stratified": has_stratification or has_variable_ratio_match,
        "inversePtWeighting": bool(ps_setting.get("inversePtWeighting")),
    }
...
def cohort_methods_spec_to_shell_recommendation(
    *,
    ...
    See docs/COHORT_METHODS_SPECIFICATIONS_RECOMMENDATION_DESIGN.md section 6.
    """
    spec = cohort_methods_spec or {}
    cspa = spec.get("createStudyPopArgs") or {}
    cmda = spec.get("getDbCohortMethodDataArgs") or {}
    ps_settings = spec.get("psSettings")
    create_ps = spec.get("createPsArgs")
    fmod = spec.get("fitOutcomeModelArgs") or {}

    primary_study_period = _first_object(cmda.get("studyPeriods"))
    if primary_study_period:
        cmda = deepcopy(cmda)
        cmda["studyStartDate"] = primary_study_period.get("studyStartDate", cmda.get("studyStartDate", ""))
        cmda["studyEndDate"] = primary_study_period.get("studyEndDate", cmda.get("studyEndDate", ""))

    primary_tar = _first_object(cspa.get("timeAtRisks"))
    if primary_tar:
        cspa = deepcopy(cspa)
        for key in ("minDaysAtRisk", "riskWindowStart", "startAnchor", "riskWindowEnd", "endAnchor"):
            if key in primary_tar:
                cspa[key] = primary_tar[key]

    primary_ps = _first_object(ps_settings)
    psadj: Dict[str, Any] = {
        "psSettings": deepcopy(ps_settings) if isinstance(ps_settings, list) else [],
        "trimByPsArgs": deepcopy(primary_ps.get("trimByPsArgs")),
        "matchOnPsArgs": deepcopy(primary_ps.get("matchOnPsArgs")),
        "stratifyByPsArgs": deepcopy(primary_ps.get("stratifyByPsArgs")),
        "createPsArgs": deepcopy(create_ps),
        "inversePtWeighting": bool(primary_ps.get("inversePtWeighting")) if primary_ps else False,
    }

    outcome_models = fmod.get("outcomeModels") if isinstance(fmod, dict) else None
    normalized_outcomes: List[Any] = []
    if isinstance(outcome_models, list):
        ps_settings_for_flags = ps_settings if isinstance(ps_settings, list) and ps_settings else [primary_ps]
        for idx, outcome_setting in enumerate(outcome_models):
            if isinstance(outcome_setting, dict):
                normalized = deepcopy(outcome_setting)
                flags = _derive_outcome_flags(
                    ps_settings_for_flags[min(idx, len(ps_settings_for_flags) - 1)]
                    if ps_settings_for_flags
                    else {}
                )
                normalized["stratified"] = flags["stratified"]
                normalized["inversePtWeighting"] = flags["inversePtWeighting"]
                normalized_outcomes.append(normalized)
            else:
                normalized_outcomes.append(deepcopy(outcome_setting))

    fmod_out = deepcopy(fmod) if isinstance(fmod, dict) else {}
    if normalized_outcomes:
        fmod_out["outcomeModels"] = normalized_outcomes
        primary_outcome_model = _first_object(normalized_outcomes)
        for key, value in primary_outcome_model.items():
            if key != "description":
                fmod_out[key] = deepcopy(value)
    else:
        flags = _derive_outcome_flags(primary_ps)
        fmod_out["stratified"] = flags["stratified"]
        fmod_out["inversePtWeighting"] = flags["inversePtWeighting"]

    study_population: Dict[str, Any] = {k: deepcopy(v) for k, v in cspa.items() if k not in _TAR_KEYS}
    if cmda:
        study_population["cohortMethodDataArgs"] = deepcopy(cmda)

    time_at_risk: Dict[str, Any] = {k: deepcopy(cspa[k]) for k in _TAR_KEYS if k in cspa}
    if isinstance(cspa.get("timeAtRisks"), list):
        time_at_risk["timeAtRisks"] = deepcopy(cspa["timeAtRisks"])
    ...
    return {
        ...
        "outcome_model": deepcopy(fmod_out),
        "deferred_inputs": {
